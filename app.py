...
# ---------------- ANALYTICS PAGE ----------------
    ...
    with col1:
        ...
        dead_stock = inventory_data[inventory_data["Risk Level"] == "High"]
        ...
        if dead_stock.empty:
            ...
            fig, ax = plt.subplots()
            ax.bar(
                dead_stock_summary["Category"],
                dead_stock_summary["Stock Quantity"]
            )
            ax.set_xlabel("Category")
            ax.set_ylabel("Stock Quantity")
            ax.set_title("Dead Stock Distribution")
...
# ---------------- REPORTS & EXPORT ----------------
    ...
    # 🔹 Example: filter by date column (change 'date' to your actual column)
    filtered_data = inventory_data[
        (inventory_data["date"] >= start) &
        (inventory_data["date"] <= end)
    ]

    if st.button("Generate Report"):
        st.success(f"Report generated: {len(filtered_data)} records")
        st.dataframe(filtered_data)

    # 🔹 CSV Export (this part was fine)
    st.download_button(
        "Export CSV",
